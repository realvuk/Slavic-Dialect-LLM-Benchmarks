...
def get_completion(client: OpenAI, model: str, prompt: str, retries: int = 3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(
                f"  empty response (finish_reason={reason}, error={err}), "
...
def predict(client: OpenAI, dataset: str, model: str, *, force: bool) -> None:
    ...
    n = len(responses)
    if n:
        print(f"  done. {elapsed / 60:.2f} min for {n} instances — {elapsed / n:.3f} s/instance.")
    else:
        ...
        json.dumps(
            {
                "system": model,
                "predictions": [
                    {"train": "NA (zero-shot)", "test": dataset, "predictions": responses}
                ],
            }
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--force", action="store_true", help="Regenerate even if a submission already exists."
    )
