...
`old_results.json` in each benchmark holds results contributed by a previous collaborator, 
Including models run locally rather than through OpenRouter. They are
**inputs**, not generated by this pipeline; `combine_results.py` tags every row
with `"Source": "new"` or `"old"` and normalises language codes so both sets
plot together.
